...
const ConsultationCard = ({ consultation }) => {
  ...
  useEffect(() => {
    ...
    const fetchPrescription = async () => {
      ...
      const response = await fetch(`http://localhost:8181/api/prescriptions/${consultation.prescriptionId}`, {
        method: 'GET',
        headers: {
    ...
    const fetchDoctorName = async () => {
      const token = localStorage.getItem('doctorjwt'); 
      const response = await fetch(`http://localhost:8181/api/doctor/${consultation.doctorId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 mb-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold text-gray-800">Consultation with Dr. {doctorName}</h3>
        <span className="text-sm text-gray-500">{new Date(consultation.date).toLocaleDateString()}</span>
      </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc pl-5 text-gray-600">
            {prescription.medicines.map((medicine, index) => (
              <li key={index}>
                <span className="font-semibold">{medicine.name}</span> - {medicine.quantity} ({medicine.dosage})
              </li>
            ))}
          </ul>
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-gray-500">Status: {consultation.status}</span>
      </div>
    </div>
  );
};
